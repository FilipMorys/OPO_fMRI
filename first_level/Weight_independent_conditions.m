...
%% CONDITIONS
conds={'StimChips','StimCucumber','StimNeutral','StimOrange','StimChocolate','PercQ','VAS'};
ons={1 2 4}; % For each condition specify which column will of the design matrix to take for each condition respectively
durs={[2 1];[3 2];[5 4]}; % For each condition to calculate durations enter which column of the time matrix sould be substracted from which
odour={[1 2 3 4 5];[0];[0]}; % Do I want to divide condition for specific odours? If yes, say which ones, if no, say 0!!!
ncond=sum(cellfun(@numel,odour)); % Or ncond=length(conds)
pmod=[1 0 0]; % parametric modulation? 1 - yes, 0 - no

%% CONTRASTS
ncon=2;
con_weights={[1 1 -4 1 1],[-1 -1 4 -1 -1]};
con_names={'StimNeu','NeuStim'};
